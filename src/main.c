...
int main(int argc, char const *argv[])
{
	...
	while (1) { //Запускаем бесконечный цикл
     ...
     int pid = fork();
        ...
        //На этом получение данных окончено, отправим наш http response и закроем соединение 
        ...
        int chat_id = atoi(strstr(header, "\"chat_id\":") + strlen("\"chat_id\":")); //То же самое что и с Content-Length
        //Осталось только отправить сообщение, для этого лучше создадим отдельную функцию
        char msg[] = "I'am fuck this ssl server";
        printf("message"); //Описание функции далее
